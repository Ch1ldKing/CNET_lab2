...
def handle_request(server_socket, initial_packet, client_address):
    ...
    if flag == 0x0010:
        ...
        # 下载请求
        filename = data.decode().strip("\x00")  # 去除填充的空字符
        print(f"[REQUEST] {client_address} 请求下载文件: {filename}")
        file_path = os.path.join("downloads", filename)
        if not os.path.exists(file_path):
            error_message = "ERROR: 文件不存在".encode()
            server_socket.sendto(error_message, client_address)
            print(f"[DOWNLOAD] 文件 '{filename}' 不存在，已通知 {client_address}")
            return
        with open(file_path, "rb") as f:
            byte_data = f.read()
        sender = Sender(server_socket, client_address, byte_data)
        sender.start()
    else:
        # 未知的请求类型
